...
/**
 * A class that encapsulates an instance of {@link org.semanticweb.owlapi.model.OWLOntology} and
 * adds an extra index mapping each instance of {@link org.semanticweb.owlapi.model.OWLIndividual}
 * to the set of all instances of {@link org.semanticweb.owlapi.model.OWLObjectPropertyAssertionAxiom}
 * in which it occurs in object position.
 *
...
public class OWLOntologyWithFurtherIndexes {
    ...
    public OWLOntologyWithFurtherIndexes(OWLOntology ontology) {
        ...
        ontology.getOWLOntologyManager()
                .addOntologyChangeListener(changes -> changes.stream().filter(
                        change -> change.isAxiomChange(OBJECT_PROPERTY_ASSERTION)).forEach(change -> {
    ...
    private void initializeLazyObjectPropertyAssertionsMap() {
        try {
            ...
            final Method buildLazyMethod = Arrays.stream(Internals.class.getDeclaredMethods())
                    .filter(method -> method.getName().equals("buildLazy")).findAny().get();

            buildLazyMethod.setAccessible(true);
            objectPropertyAssertionsByObjectIndividual =
                    (MapPointer<OWLIndividual, OWLObjectPropertyAssertionAxiom>) buildLazyMethod.invoke(
                            ints,
                            OBJECT_PROPERTY_ASSERTION,
                            INDIVIDUALSUPERNAMED,
    ...
    public final Stream<OWLObjectPropertyAssertionAxiom> objectPropertyAssertionAxiomsWithSubject(OWLIndividual individual) {
        // The OWLAPI already manages an index for the desired results, so
        // there is no need to create a second one.
