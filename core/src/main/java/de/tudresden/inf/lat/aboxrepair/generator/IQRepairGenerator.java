...
public class IQRepairGenerator extends RepairGenerator {
    ...
    /**
     *  The below code is a bit difficult to understand, but I believe that the current
     *  version now does the job correctly.
     */
    protected void generateVariables() {
        ...
        while(!queueOfIndividuals.isEmpty()) {
            ...
            OWLIndividual originalIndividual =
                    inputObjectNames.contains(individual) ? individual : copyToObject.get(individual);

            Set<OWLObjectPropertyAssertionAxiom> setOfRoleAssertions =
                    ontology.getObjectPropertyAssertionAxioms(originalIndividual);
            ...
            for(OWLObjectPropertyAssertionAxiom roleAssertion : setOfRoleAssertions) {
                ...
                Set<OWLClassExpression> successorSet =
                        computeSuccessorSet(type, (OWLObjectProperty) roleAssertion.getProperty(), originalObject);

                RepairType emptyType = typeHandler.newMinimisedRepairType(new HashSet<>());
                Set<RepairType> setOfRepairTypes =
                        typeHandler.findCoveringRepairTypes(emptyType, successorSet, originalObject);
