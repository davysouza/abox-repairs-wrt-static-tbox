package de.tudresden.inf.lat.aboxrepair.reasoner;
...
import de.tudresden.inf.lat.aboxrepair.utils.DisposableFinalVariable;
...
public class ReasonerFacade {
    ...
    private final FreshOWLEntityFactory.FreshOWLClassFactory freshOWLClassFactory;
    private final FreshOWLEntityFactory.FreshOWLNamedIndividualFactory freshOWLNamedIndividualFactory;
    // private final Set<OWLClassExpression> expressions; // TODO only for testing - remove afterwards!
    // private Timer timer;

    /**
     * Uses no TBox, and only supports the expressions provided. Hence, the manager is required to create the ontology
     * used internally.
     */
    public static ReasonerFacade newReasonerFacadeWithoutTBox(Set<OWLClassExpression> expressions, OWLOntologyManager manager)
    ...
    /**
     * Uses no TBox for reasoning, however, supports the expressions found in the ontology (subsumption relations
     * without TBox).
     *
     * @param ontology
     * @return
     * @throws OWLOntologyCreationException
     */
    public static ReasonerFacade newReasonerFacadeWithoutTBox(OWLOntology ontology) throws OWLOntologyCreationException {
     ...
     * @throws OWLOntologyCreationException
     */
    public static ReasonerFacade newReasonerFacadeWithoutTBox(OWLOntology ontology, Set<OWLClassExpression> additionalExpressions)
            throws OWLOntologyCreationException {
     ...
     * @return
     */
    public static ReasonerFacade newReasonerFacadeWithTBox(OWLOntology ontology, Set<OWLClassExpression> additionalExpressions) {
//        Set<OWLClassExpression> expressions = ontology.getNestedClassExpressions();
...
//        addedAxioms = new HashSet<>();
        ...
        this.freshOWLClassFactory = FreshOWLEntityFactory.FreshOWLClassFactory.of(ontologyCopy.get());
        this.freshOWLClassFactory.addAdditionalKnownEntities(expressions);
        this.freshOWLNamedIndividualFactory = FreshOWLEntityFactory.FreshOWLNamedIndividualFactory.of(ontologyCopy.get());
...
//        alreadyAdded.clear();
    ...
    public final Stream<OWLClassExpression> types(OWLIndividual ind) throws IllegalArgumentException {
        verifyKnows(ind);
        return reasoner.types(freshOWLNamedIndividualFactory.getEntity(ind)).map(name -> freshOWLClassFactory.getObject(name));
    }
    ...
    public final Stream<OWLClassExpression> equivalentClasses(OWLClassExpression exp) throws IllegalArgumentException {
        verifyKnows(exp);
        return reasoner.equivalentClasses(freshOWLClassFactory.getEntity(exp)).map(name -> freshOWLClassFactory.getObject(name));
    }
