    ...
    public RepairType minimise(RepairType type) {
        ...
        for(OWLClassExpression exp:type.getClassExpressions()){
            ...
            if(newClasses.contains(exp)) {
            	newClasses.removeAll(reasonerWithoutTBox.equivalentIncludingOWLThingAndOWLNothingOrStrictlySubsumedByExcludingOWLThingAndOWLNothing(exp));
                newClasses.add(exp);
            }
        }
...
//        return true;
        return repairPreType.stream().parallel()
                .allMatch(atom ->
                        reasonerWithTBox.equivalentIncludingOWLThingAndOWLNothingOrStrictlySubsumedByExcludingOWLThingAndOWLNothing(atom).stream().parallel()
                                .allMatch(subsumee ->
                                        !reasonerWithTBox.instanceOf(ind, subsumee)
    ...
    *  'successorSet' whatsoever.  See also my comment to the below method 'findCoveringPreTypes'. */
    public Set<RepairType> findCoveringRepairTypes(RepairType repairType,
    		Set<OWLClassExpression> successorSet, OWLIndividual ind) {
